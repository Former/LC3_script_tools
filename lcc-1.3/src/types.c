...
static char rcsid[] = "$Id: types.c,v 1.1.1.1 2004/03/24 04:37:33 sjp Exp $";
...
static Type xxinit(int op, char *name, Metrics m) {
	...
	switch (ty->op) {
	case INT:
		p->u.limits.max.i = ones(m.bits_count_in_one_byte * ty->size)>>1;
		p->u.limits.min.i = -p->u.limits.max.i - 1;
		break;
	case UNSIGNED:
		p->u.limits.max.u = ones(m.bits_count_in_one_byte * ty->size);
		p->u.limits.min.u = 0;
		break;
	case FLOAT:
...
void type_init(int argc, char *argv[]) {
	...
	if (!IR)
		return;
	for (i = 1; i < argc; i++) {
		int size, align, outofline, bits_count_in_one_byte;
